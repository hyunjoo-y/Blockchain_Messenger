...
class FriendsListDBHelper(val context: FragmentActivity) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    private var insertCheck: Boolean = true

    companion object {
        const val DB_VERSION = 1
        const val DB_NAME = "friendsList.db"
    ...
    override fun onCreate(db: SQLiteDatabase?) {
         val create_table = "create table if not exists $TABLE_NAME($IMAGE" +
                 " blob,  $NAME text, $ID text, $STATUS text, $LIB_ADDR text, $ADDRESS text)"
    ...
    fun insertFriends(proData: ProfileData): Boolean {
...
/*
    ...
    fun readData(): MutableList<ProfileData> {
        ...
        val query = "Select * from $TABLE_NAME order by name desc"
